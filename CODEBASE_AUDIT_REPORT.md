...
| `PheromoneGrid.ClearEnemyTrailForTarget` | `Simulation/Pheromones/PheromoneGrid.cs:233` | **Live cleanup method** (audit corrected 2026-04-18). Called on colony death via `World.DespawnDeadColonies` → `ForgetEnemyTrailAboutDeadColony` (`World.cs:372`). Removes the dead colony's keys from 4 per-enemy Dictionaries (`_enemyTrails`, `_enemyDistances`, `_enemyActiveCells`, `_enemyActiveSet`). Prevents memory leak + stale enemy-trail reads by survivors. **NOT dead.** | Repo-wide grep → 1 live consumer at `World.cs:372` inside the colony-death cleanup path. |
